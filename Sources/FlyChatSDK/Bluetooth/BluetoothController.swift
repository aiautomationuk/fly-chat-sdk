...
class BluetoothController: NSObject {
  ...
  lazy private var log = OSLog(subsystem: label, category: "FlyChatSDK")
  
  lazy private var dispatchQueue: DispatchQueue =
    DispatchQueue(label: label, qos: .userInteractive)
  
  weak var service: FlyChatBluetoothService?
  
  private var centralManager: CBCentralManager?
  
  private var discoveredPeripherals = Set<CBPeripheral>()
  
  private var discoveredPeripheralsWithFlyChatServices: Set<CBPeripheral> {
  ...
  private var servicesOfPeripherals =
    [CBPeripheral : Set<FlyChatBluetoothService>]()
  ...
  private func _send(
    ...
    guard self.discoveredPeripheralsWithFlyChatServices.count > 0 else {
      return
    }
    ...
    let actions = {
      if !isInBackground,
        ...
        self.discoveredPeripheralsWithFlyChatServices.forEach {
...
extension BluetoothController: CBPeripheralDelegate {
  ...
  func peripheral(
    ...
    do {
      ...
      let remoteService = try FlyChatBluetoothService(
        isLocal: false,
