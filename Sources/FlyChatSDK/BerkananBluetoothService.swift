...
/// A protocol defining methods that local FlyChat Bluetooth service instances call on their delegates to
/// handle events like receiving messages.
public protocol FlyChatBluetoothServiceDelegate: class {
  
  /// Tells the delegate that the local FlyChat Bluetooth `service` discovered `remoteService` with
  ...
  func berkananBluetoothService(
    _ service: FlyChatBluetoothService,
    didDiscover remoteService: FlyChatBluetoothService
  )
  
  /// Tells the delegate that the local FlyChat Bluetooth `service` received a `message`.
  ///
  /// - Parameters:
...
/// An object that represents a FlyChat Bluetooth service hosted by an app running
...
public class FlyChatBluetoothService: NSObject {
  
  /// The delegate assigned with this object.
  ///
  /// Note: Events are not delivered on the main thread.
  public weak var delegate: FlyChatBluetoothServiceDelegate?
  
  #if canImport(Combine)
  
  private lazy var _discoverServiceSubjectAny: Any = {
    if #available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *) {
      return PassthroughSubject<FlyChatBluetoothService, Never>()
    }
    return NSObject()
  }()
  ...
  public var discoverServiceSubject: PassthroughSubject<FlyChatBluetoothService, Never> {
    _discoverServiceSubjectAny as! PassthroughSubject<FlyChatBluetoothService, Never>
  }

  private lazy var _numberOfServicesInRangeSubjectAny: Any = {
    if #available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *) {
  ...
  /// Note: Events are not delivered on the main thread.
  @available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
  public var numberOfServicesInRangeSubject: PassthroughSubject<Int, Never> {
    _numberOfServicesInRangeSubjectAny as! PassthroughSubject<Int, Never>
  }

  private lazy var _receiveMessageSubjectAny: Any = {
    if #available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *) {
      return PassthroughSubject<Message, Never>()
    }
    return NSObject()
  }()
  ...
  /// Note: Events are not delivered on the main thread.
  @available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
  public var receiveMessageSubject: PassthroughSubject<Message, Never> {
    _receiveMessageSubjectAny as! PassthroughSubject<Message, Never>
  }
  ...
  public internal(set) var servicesInRange = Set<FlyChatBluetoothService>() {
    didSet {
      #if canImport(Combine)
      if #available(OSX 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *) {
  ...
  public func send(
    _ message: Message,
    via services: [FlyChatBluetoothService]? = nil,
