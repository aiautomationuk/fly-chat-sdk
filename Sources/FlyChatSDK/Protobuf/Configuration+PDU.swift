...
import Foundation
import Compression
import FlyChatCompression
import CoreBluetooth.CBError

extension Configuration {
  
  /// Initializes `self` with `pdu` representation, which is expected to be a serialized
  /// `Configuration` instance compressed using ZLIB algorithm.
  /// 
